  ...
  try {
    ...
    if (operation === 'get') {
      const command = new GetObjectCommand({
        ...
        Key: fileName, // fileName here is the S3 Key
      });
      const url = await getSignedUrl(s3Client, command, { expiresIn: 3600 });
      return NextResponse.json({ url });
    }

    // Generate unique key following the naming convention expected by Lambda
    // Format: studentID_subject_timestamp_originalfilename.ext
